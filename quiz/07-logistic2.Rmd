...
`r longmcq(c("Separate models for each sex",
             answer = "An interaction term: age * sex",
             "Only main effects: age + sex",
...
`r longmcq(c("Young children who died",
             answer = "Older men who survived and older women who died",
             "All women regardless of outcome",
             "Young women who survived"))`
...
`r longmcq(c("Different slopes and different intercepts for each logit",
             "Different slopes but equal intercepts for each logit",
             answer = "Equal slopes but different intercepts for each logit",
             "Equal slopes and equal intercepts for all logits"))`
...
`r longmcq(c("glm() with family = binomial",
             answer = "MASS::polr()",
             "nnet::multinom()",
             "VGAM::vglm() with parallel = FALSE"))`
...
`r longmcq(c("A model with only main effects",
             "A simpler model with fewer predictors",
             answer = "A non-proportional odds (NPO) model that allows different slopes",
             "A nested dichotomies model"))`
...
`r longmcq(c("None vs. Some, Some vs. Marked",
             answer = "None vs. (Some or Marked), Some vs. Marked",
             "None vs. Some, None vs. Marked",
             "All three categories vs. each other"))`
...
`r longmcq(c("Probabilities of each response category",
             "Odds ratios between all pairs of categories",
             answer = "Log odds of category j vs. the reference category m",
             "The change in probability for a unit change in the predictor"))`
...
`r longmcq(c("The response categories are random",
             answer = "An unobserved continuous variable is cut into discrete categories by thresholds",
             "The predictors are unmeasured",
             "The model requires hidden variables to be estimated"))`
