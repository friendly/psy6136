...
`r longmcq(c("Continuous and normally distributed",
             answer = "Binary (0/1, success/failure, yes/no)",
             "Count data following a Poisson distribution",
             "Categorical with more than two unordered categories"))`
...
`r longmcq(c("OLS cannot handle categorical predictors",
             answer = "OLS predictions can fall outside the [0, 1] range and violates homogeneity of variance",
             "OLS is too computationally intensive for binary data",
             "OLS requires a larger sample size than logistic regression"))`
...
`r longmcq(c("logit(π) = π / (1 - π)",
             answer = "logit(π) = log[π / (1 - π)]",
             "logit(π) = 1 / (1 + exp(-π))",
             "logit(π) = exp(π)"))`
...
`r longmcq(c("The change in probability for a unit increase in the predictor",
             answer = "The change in log odds for a unit increase in the predictor",
             "The odds ratio for the predictor",
             "The correlation between the predictor and response"))`
...
`r longmcq(c("lm(outcome ~ age + sex)",
             answer = "glm(outcome ~ age + sex, family = binomial)",
             "loglm(outcome ~ age + sex)",
             "logit(outcome ~ age + sex)"))`
...
`r longmcq(c("Ordinary least squares (OLS)",
             ...
             "Bayesian estimation with non-informative priors"))`
...
`r longmcq(c("F-tests",
             answer = "Wald z-tests or χ² tests",
             "t-tests",
             "ANOVA F-tests"))`
...
`r longmcq(c("For each predictor separately, ignoring all other predictors",
             answer = "For high-order terms in the model, averaged over (controlling for) other predictors",
             "Only for the intercept term",
             "Only for interaction terms"))`
...
`r longmcq(c("High leverage only (unusual predictor values)",
             "Large residual only (poor fit)",
             answer = "Both high leverage and large residual (unusual in X and Y)",
             "Small Cook's D statistic"))`
